...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        //出售中产品
        $onsale =  ProductModel::where(['is_show' => 1, 'is_del' => 0,'type'=>1])->count();
        //待上架产品
        $forsale =  ProductModel::where(['is_show' => 0, 'is_del' => 0,'type'=>1])->count();
        //仓库中产品
        $warehouse =  ProductModel::where(['is_del' => 0,'type'=>1])->count();
        //已经售馨产品
        $outofstock = ProductModel::getModelObject()->where(ProductModel::setData(4,1))->count();
        //警戒库存
        $policeforce = ProductModel::getModelObject()->where(ProductModel::setData(5,1))->count();
        //回收站
        $recycle =  ProductModel::where(['is_del' => 1,'type'=>1])->count();
    ...
    public function save_item(Request $request)
    {
        ...
        if (count($data['open_address']) < 1) return Json::fail('请填写所在区域');
