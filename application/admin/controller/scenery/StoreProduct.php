...
use app\admin\model\store\StorePack as PackModel;
...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        //出售中产品
        $onsale =  ProductModel::where(['is_show' => 1, 'is_del' => 0,'type'=>2 ])->count();
        //待上架产品
        $forsale =  ProductModel::where(['is_show' => 0, 'is_del' => 0,'type'=>2 ])->count();
        //仓库中产品
        $warehouse =  ProductModel::where(['is_del' => 0 ,'type'=>2 ])->count();
        //已经售馨产品
        $outofstock = ProductModel::getModelObject()->where(ProductModel::setData(4,2))->count();
        //警戒库存
        $policeforce = ProductModel::getModelObject()->where(ProductModel::setData(5,2))->count();
        //回收站
        $recycle =  ProductModel::where(['is_del' => 1,'type'=>2 ])->count();
    ...
    public function create_item()
    {
        $field = [
            ...
            Form::select('pack_id', '包装礼盒')->setOptions(function () {
                $list = PackModel::getTierList();
                $menus = [];
                foreach ($list as $menu) {
            ...
            Form::input('area', '种植面积', '1㎡'),
            Form::frameImageOne('image', '产品主图片(305*305px)', Url::build('admin/widget.images/index', array('fodder' => 'image')))->icon('image')->width('50%')->height('500px'),
            Form::frameImageOne('exchange_image', '兑换商品图片(305*305px)', Url::build('admin/widget.images/index', array('fodder' => 'exchange_image')))->icon('image')->width('50%')->height('500px'),
            ...
            Form::city('open_address', '所在区域')->type('city_area')->trigger('click')->filterable(true),
            ...
            Form::number('exchange_rule', '兑换需茶叶量')->min(0)->col(8),
            Form::number('production_rate', '茶叶产率 (1片/秒)')->min(0)->col(8),
        ...
        $form = Form::make_post_form('添加茶树', $field, Url::build('save_item'), 2);
    ...
    public function save_item(Request $request)
    {
        ...
        if ($data['mer_id']==0) return Json::fail('请选择所属店铺');
        if ($data['pack_id']==0) return Json::fail('请选择包装礼盒类型');

        if (!$data['store_name']) return Json::fail('请输入产品名称');

        if ($data['image'][0] =='' ) return Json::fail('请上传产品图片');
        if ($data['exchange_image'][0] == '') return Json::fail('请上传兑换商品图片');

        if (count($data['slider_image']) < 1) return Json::fail('请上传产品轮播图');
        if(count($data['slider_image'])>5) return Json::fail('轮播图最多5张图');

        if (count($data['open_address']) < 1) return Json::fail('请填写种植地区');
        ...
        if ($data['exchange_rule'] == '' || $data['exchange_rule'] < 0) return Json::fail('请输入兑换需茶叶量');
        $data['open_address'] = $data['open_address'][0].'/'.$data['open_address'][1];
        $data['image'] = $data['image'][0];

        $data['exchange_image'] = $data['exchange_image'][0];
    ...
    public function edit($id, $type = '')
    {
        ...
        $field = [
            ...
            Form::select('pack_id', '包装礼盒',(string) $product->getData('pack_id'))->setOptions(function () {
                $list = PackModel::getTierList();
                $menus = [];
                foreach ($list as $menu) {
                    $menus[] = ['value' => $menu['id'], 'label' => $menu['name']];
            ...
            Form::input('area', '种植面积/棵', $product->getData('area')),
            Form::input('unit_name', '产品单位', $product->getData('unit_name')),
            Form::frameImageOne('image', '产品主图片(305*305px)', Url::build('admin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('50%')->height('500px'),
            Form::frameImageOne('exchange_image', '兑换商品图片(305*305px)', Url::build('admin/widget.images/index', array('fodder' => 'exchange_image')), $product->getData('exchange_image'))->icon('image')->width('50%')->height('500px'),
            ...
            Form::city('open_address', '所在区域')->trigger('click')->filterable(true)->placeholder($product->getData('open_address')),
            ...
            Form::number('exchange_rule', '兑换需茶叶量', $product->getData('exchange_rule'))->min(0)->col(8),
            Form::number('production_rate', '茶叶产率 (1片/秒)', $product->getData('production_rate'))->min(0)->col(8),
  ...
  /**
    ...
    public function update_item(Request $request, $id)
    {
        ...
        if (!$data['mer_id']) return Json::fail('请选择所属店铺');
        if (!$data['pack_id']) return Json::fail('请选择礼盒包装类型');
