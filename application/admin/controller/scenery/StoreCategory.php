<?php
namespace app\admin\controller\scenery;

use app\admin\controller\AuthController;
use service\FormBuilder as Form;
use service\JsonService;
use service\UtilService as Util;
use service\JsonService as Json;
use service\UploadService as Upload;
use think\Request;
use app\admin\model\store\StoreCategory as CategoryModel;
use app\admin\model\store\StorePack as PackModel;
use think\Url;
use app\admin\model\system\SystemAttachment;

/**
 * 产品分类控制器
 * Class StoreCategory
 * @package app\admin\controller\system
 */
class StoreCategory extends AuthController
{

    /**
     * 显示资源列表
     *
     * @return \think\Response
     */
    public function index()
    {
        return $this->fetch();
    }
    /*
     *  异步获取分类列表
     *  @return json
     */
    public function category_list(){
        $where = Util::getMore([
            ['is_show',''],
            ['name',''],
            ['page',1],
            ['limit',20],
            ['order','']
        ]);
        return JsonService::successlayui(PackModel::CategoryList($where));
    }
    /**
     * 设置单个产品上架|下架
     *
     * @return json
     */
    public function set_show($is_show='',$id=''){
        ($is_show=='' || $id=='') && JsonService::fail('缺少参数');
        $res=PackModel::where(['id'=>$id])->update(['is_show'=>(int)$is_show]);
        if($res){
            return JsonService::successful($is_show==1 ? '显示成功':'隐藏成功');
        }else{
            return JsonService::fail($is_show==1 ? '显示失败':'隐藏失败');
        }
    }
    /**
     * 快速编辑
     *
     * @return json
     */
    public function set_category($field='',$id='',$value=''){
        $field=='' || $id=='' || $value=='' && JsonService::fail('缺少参数');
        if(CategoryModel::where(['id'=>$id])->update([$field=>$value]))
            return JsonService::successful('保存成功');
        else
            return JsonService::fail('保存失败');
    }
    /**
     * 显示创建资源表单页.
     *
     * @return \think\Response
     */
    public function create()
    {
        $field = [
            Form::input('name','礼盒名称'),
            Form::frameImageOne('pack_image','包装盒图片',Url::build('admin/widget.images/index',array('fodder'=>'pack_image')))->icon('image'),
            Form::number('sort','排序'),
            Form::number('pack_cost','价钱'),
            Form::input('describe', '包装盒描述')->type('textarea'),
            Form::radio('is_show','状态',1)->options([['label'=>'显示','value'=>1],['label'=>'隐藏','value'=>0]])
        ];
        $form = Form::make_post_form('添加分类',$field,Url::build('save'),2);
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }


    /**
     * 上传图片
     * @return \think\response\Json
     */
    public function upload()
    {
        $res = Upload::image('file','store/category'.date('Ymd'));
        if(is_array($res)){
            SystemAttachment::attachmentAdd($res['name'],$res['size'],$res['type'],$res['dir'],$res['thumb_path'],1,$res['image_type'],$res['time']);
            return Json::successful('图片上传成功!',['name'=>$res['name'],'url'=>Upload::pathToUrl($res['thumb_path'])]);
        }else
            return Json::fail($res);
    }


    /**
     * 保存新建的资源
     *
     * @param  \think\Request  $request
     * @return \think\Response
     */
    public function save(Request $request)
    {

        $data = Util::postMore([
            'name',
            'pack_cost',
            'describe',
            'sort',
            ['pack_image',[]],
            ['is_show',0]
        ],$request);
        if(!$data['name']) return Json::fail('请输入包装盒名称');
        if($data['pack_image'][0]=='')return Json::fail('请上传包装盒图片');
        if($data['sort'] <0 ) $data['sort'] = 0;
        $data['pack_cost']=$data['pack_cost'];
        $data['describe']=$data['describe'];
        $data['is_show']=$data['is_show'];
        $data['pack_image'] = $data['pack_image'][0];
        $data['add_time'] = time();
        PackModel::set($data);
        return Json::successful('添加包装盒成功!');
    }

    /**
     * 显示编辑资源表单页.
     *
     * @param  int  $id
     * @return \think\Response
     */
    public function edit($id)
    {
        $c = PackModel::get($id);
        if(!$c) return Json::fail('数据不存在!');
        $field = [

            Form::input('name','礼盒名称',$c->getData('name')),
            Form::frameImageOne('pack_image','包装盒图片',Url::build('admin/widget.images/index',array('fodder'=>'pack_image')),$c->getData('pack_image'))->icon('image'),
            Form::number('sort','排序',$c->getData('sort')),
            Form::number('pack_cost','价钱',$c->getData('pack_cost')),
            Form::input('describe', '包装盒描述',$c->getData('describe'))->type('textarea'),
            Form::radio('is_show','状态',$c->getData('is_show'))->options([['label'=>'显示','value'=>1],['label'=>'隐藏','value'=>0]])
        ];
        $form = Form::make_post_form('编辑分类',$field,Url::build('update',array('id'=>$id)),2);

        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }

    /**
     * 保存更新的资源
     *
     * @param  \think\Request  $request
     * @param  int  $id
     * @return \think\Response
     */
    public function update(Request $request, $id)
    {
        $data = Util::postMore([
            'name',
            'pack_cost',
            'describe',
            'sort',
            ['pack_image',[]],
            ['is_show',0]
        ],$request);
        if(!$data['name']) return Json::fail('请输入包装盒名称');
        if($data['pack_image'][0]=='')return Json::fail('请上传包装盒图片');
        if($data['sort'] <0 ) $data['sort'] = 0;
        $data['pack_cost']=$data['pack_cost'];
        $data['describe']=$data['describe'];
        $data['is_show']=$data['is_show'];
        $data['pack_image'] = $data['pack_image'][0];
        $data['add_time'] = time();
        PackModel::edit($data,$id);
        return Json::successful('修改成功!');
    }

    /**
     * 删除指定资源
     *
     * @param  int  $id
     * @return \think\Response
     */
    public function delete($id)
    {
        if(!PackModel::delCategory($id))
            return Json::fail(PackModel::getErrorInfo('删除失败,请稍候再试!'));
        else
            return Json::successful('删除成功!');
    }
}
