 ...
 * 文章分类管理  控制器
 * */
class ScenicCategory extends AuthController

{
    ...
    public function index(){
        ...
        $this->assign(ScenicCategoryModel::systemPage($where));
        return $this->fetch();
    }
...
//        })->multiple(1)->filterable(1);
    ...
    public function save(Request $request){
        ...
        $new_id = $data['new_id'];
        unset($data['new_id']);
        $res = ScenicCategoryModel::set($data);
        if(!ScenicModel::saveBatchCid($res['id'],implode(',',$new_id))) return Json::fail('文章列表添加失败');
    ...
    public function edit($id){
        ...
        $article =ScenicCategoryModel::get($id)->getData();
...
//            ['new_id',[]],
        ...
        if($data['sort'] < 0) return Json::fail('排序不能是负数');
        $data['image'] = $data['image'][0];
        if(!ScenicCategoryModel::get($id)) return Json::fail('编辑的记录不存在!');
        ScenicCategoryModel::edit($data,$id);
    ...
    {
        $res = ScenicCategoryModel::delArticleCategory($id);
        if(!$res)
            return Json::fail(ScenicCategoryModel::getErrorInfo('删除失败,请稍候再试!'));
        else
