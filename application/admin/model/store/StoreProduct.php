...
//        return $res;
     ...
     * @return array
     */
    public static function setData($type,$type_val=''){
        switch ((int)$type){
            case 1:
            ...
            case 3:
                ...
                break;
            case 4:
                $data = empty($type_val)? ['p.is_show'=>1,'p.is_del'=>0,'pav.stock|p.stock'=>0] : ['p.is_show'=>1,'p.is_del'=>0,'pav.stock|p.stock'=>0,'p.type'=>$type_val];
                break;
            case 5:
                $min = SystemConfig::getValue('store_stock');
                $data = empty($type_val)? ['p.is_show'=>1,'p.is_del'=>0,'pav.stock|p.stock'=>['elt',$min]]:['p.is_show'=>1,'p.is_del'=>0,'pav.stock|p.stock'=>['elt',$min],'p.type'=>$type_val];
                break;
            case 6:
                $data = ['p.is_del'=>1];
                break;
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            if(isset($where['field_type']) && $where['field_type']!=''){
                $model = $model->where('type',$where['field_type']);
            }
            if(isset($where['type']) && $where['type']!='' && ($data=self::setData($where['type']))){          //由于这里调用了setData()所以该方法需要进行是否存在type_val判断
