...
class StoreCart extends ModelBasic
{
    ...
    /**
     * @Modify: Mr. Lin
     * @function: 秒杀、拼团、立即购买、加入购物车...
     * @instructions: 
     * @param {type} 
     * @return: JSON
     */
    public static function setCart($uid,$product_id,$mer_id,$cart_num = 1,$product_attr_unique = '',$type='product',$is_new = 0,$combination_id=0,$seckill_id = 0,$bargain_id = 0,$immediately=true)
    {
        ...
        if($seckill_id){
            ...
            if($cart = self::where($where)->find()){
                ...
                return self::set(compact('uid','product_id','mer_id','cart_num','product_attr_unique','is_new','type','seckill_id'));
            }
        }elseif($bargain_id){
            ...
            if($cart = self::where($where)->find()){
                ...
                return self::set(compact('uid','product_id','mer_id','cart_num','product_attr_unique','is_new','type','bargain_id'));
            }
        ...
        if($cart = self::where($where)->find()){
            ...
            return self::set(compact('uid','product_id','mer_id','cart_num','product_attr_unique','is_new','type','combination_id'));
        }
    ...
    {
        $productInfoField = 'id,mer_id,image,slider_image,price,ot_price,vip_price,postage,mer_id,give_integral,cate_id,sales,stock,store_name,store_info,unit_name,is_show,is_del,is_postage,cost,type,open_address,exchange_image,pack_id';
        ...
        foreach ($list as $k=>$cart){
            ...
            if($product['pack_id']!=0){             //礼品盒信息
                $store_pack = StorePack::where('id',$product['pack_id'])->find();
                $cart['storePack'] = $store_pack;
            }
            ...
            if(!$product){
                ...
                $product['open_address']=json_decode($product['open_address']);
    ...
    public static function getUserProductCartListApi($uid,$cartIds='',$status=0)
    {
        $productInfoField = 'id,mer_id,image,slider_image,price,ot_price,vip_price,postage,mer_id,give_integral,cate_id,sales,stock,store_name,store_info,unit_name,is_show,is_del,is_postage,cost';
        $seckillInfoField = 'id,mer_id,image,price,ot_price,postage,give_integral,sales,stock,title as store_name,unit_name,is_show,is_del,is_postage,cost';
        $bargainInfoField = 'id,mer_id,image,min_price as price,price as ot_price,postage,give_integral,sales,stock,title as store_name,unit_name,status as is_show,is_del,is_postage,cost';
        $combinationInfoField = 'id,mer_id,image,price,postage,sales,stock,title as store_name,is_show,is_del,is_postage,cost';
        $model = new self();
        $valid = $invalid = [];
        $model = $model->field('a.*,b.store_name as merchant_name')->where('uid',$uid)->where('a.type','product')->where('is_pay',0)
            ->where('is_del',0)->alias("a")->join('store_merchant b','a.mer_id = b.id','left');
        if(!$status) $model->where('is_new',0);
        if($cartIds) $model->where('id','IN',$cartIds);
        $list = $model->select()->toArray();
        if(!count($list)) return compact('valid','invalid');
        ...
        foreach($valid as $key => $value){
            if(!in_array($value['mer_id'],$data_id)){
                ...
                $data[$value['mer_id']]['list'][] = $value;
            }
        }
        $valid = array_values($data);
