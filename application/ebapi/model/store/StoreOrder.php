...
class StoreOrder extends ModelBasic
{
    ...
    /**
     * @Notes:type为1时是商品购买,type为2时是茶叶兑换商品只需要收取包装费用
     * @author Handsome Lin
     * @date 2020/2/1 20:12
     *
     */
    public static function getOrderPriceGroup($cartInfo,$type=1)
    {
        ...
        $totalPrice = self::getOrderSumPrice($cartInfo,'truePrice',$type);//获取订单总金额
        $costPrice = self::getOrderSumPrice($cartInfo,'costPrice',$type);//获取订单成本价
        $vipPrice = self::getOrderSumPrice($cartInfo,'vip_truePrice',$type);//获取订单会员优惠金额
        //如果满额包邮等于0
        if(!$storeFreePostage) {
            $storePostage = 0;
        }else{
...
//        $totalPrice = bcadd($totalPrice,$storePostage,2);
    ...
    public static function getOrderSumPrice($cartInfo,$key='truePrice',$type=1)
    {
        $SumPrice = 0;
        foreach ($cartInfo as $cart){
            if($type==3){                               //TODO 需要获取包装礼盒的价钱,暂时设为5块一个
                $store_product=StoreProduct::where('id',$cart['productInfo']['id'])->field('pack_id')->find();
                $store_pack=StorePack::where('id',$store_product['pack_id'])->field('pack_cost')->find();
                $SumPrice = bcadd($SumPrice,bcmul($cart['cart_num'],$store_pack['pack_cost'],2),2);
            }else{
     ...
     * @return bool|object
     */
    public static function cacheKeyCreateOrder($uid,$key,$addressId,$payType,$useIntegral = false,$couponId = 0,$mark = '',$combinationId = 0,$pinkId = 0,$seckill_id=0,$bargain_id=0,$type=1,$date='',$open_address='',$production_rate='3600')
    {
        ...
        $payPrice = (float)$priceGroup['totalPrice'];            //获取商品的价钱

        $payPostage = $priceGroup['storePostage'];
        $addressInfo= ['real_name'=>'','phone'=>'','province'=>'','city'=>'','district'=>'','detail'=>''];
        if(!$addressId && $type!=2) return self::setErrorInfo('请选择收货地址!');
        if($type!=2 && (!UserAddress::be(['uid'=>$uid,'id'=>$addressId,'is_del'=>0]) || !($addressInfo = UserAddress::find($addressId))))
...
//        }
    ...
    {
        $noBuy = self::where('uid',$uid)->where('paid',0)->where('is_del',0)->where('type','<>',2)->where('pay_type','<>','offline')->count();
        $noPostageNoPink = self::where('o.uid',$uid)->alias('o')->where('o.paid',1)->where('o.pink_id',0)->where('type','<>',2)->where('o.is_del',0)->where('o.status',0)->where('o.pay_type','<>','offline')->count();
        $noPostageYesPink = self::where('o.uid',$uid)->alias('o')->join('StorePink p','o.pink_id = p.id')->where('type','<>',2)->where('p.status',2)->where('o.paid',1)->where('o.is_del',0)->where('o.status',0)->where('o.pay_type','<>','offline')->count();
        ...
        $noReply = self::where('uid',$uid)->where('paid',1)->where('is_del',0)->where('type','<>',2)->where('status',2)->count();
        $noPink = self::where('o.uid',$uid)->alias('o')->join('StorePink p','o.pink_id = p.id')->where('p.status',1)->where('type','<>',2)->where('o.paid',1)->where('o.is_del',0)->where('o.status',0)->where('o.pay_type','<>','offline')->count();
        $noRefund = self::where('uid',$uid)->where('paid',1)->where('is_del',0)->where('refund_status','IN','1,2')->where('type','<>',2)->count();
    ...
    public static function getUserTreeCount(){
     ...
     * @return array
     * */
    public static function getUserOrderSearchList($uid,$status,$page,$limit,$search)
    {
        ...
        if($search){
            ...
            $list = self::getUserOrderList($uid,$status,$page,$limit);
        }
        
     
        foreach ($list as $k=>$order){
            $list[$k] = self::tidyOrder($order,true);
            $list[$k]['store_name']=Db::table('eb_store_merchant')->where('id','=',$order['mer_id'])->find()['store_name'];
    ...
    public static function watchPayStatus($order_id){
        return self::where('order_id',$order_id)->field('paid')->find();
    }
    public static function updatePayStatus($order_id){
        return self::where('order_id',$order_id)->update(['paid'=>1,'pay_time'=>time(),'status'=>2,'production_time'=>time()]);
    }

    public static function getPayTimeById($val){
        $where['id'] = array('in', explode(',',$val));
        $array=array();
        $obj=self::where($where)->where('status',2)->field('production_time,production_rate')->select();
        return $obj;
    }
}