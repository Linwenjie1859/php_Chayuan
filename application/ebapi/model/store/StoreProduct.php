...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
        ...
        $list=$model->page((int)$page,(int)$limit)->field('id,store_name,cate_id,image,ficti as sales,price,stock,store_info,ot_price')->select();
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public static function getBestProduct($field = '*',$limit=0,$first=0,$uid=0)
    {
        $model = self::where('type',1)->where('is_del',0)
            ->where('stock','>',0)->where('is_show',1)->field($field)
            ->order('sort DESC,sales DESC');
        if($limit && !$first) $model->limit($limit);
        else{
            $model->page((int)$first,(int)$limit)->field('*');
        }
        return self::setLevelPrice($model->select(),$uid);
    }
     ...
     * @return: JSON
     */
    public static function getOtherProduct($field = '*',$limit=0,$first=0,$uid=0)
    {
        $model = self::where('type',2)->where('is_del',0)
            ->where('stock','>',0)->where('is_show',1)->field($field)
            ->order('sort DESC ,sales DESC');
        if($limit && !$first) $model->limit($limit);
        else{
     ...
     * @Notes:
     */
    public static function getTreeDetailsById($product_id,$field='*'){
        return self::where('id',$product_id)->field($field)->find();
    }
