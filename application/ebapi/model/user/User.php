...
class User extends ModelBasic
{
    ...
    public static function updateWechatUser($wechatUser,$uid)
    {
        ...
        if($userinfo->spread_uid){
            ...
            if(isset($wechatUser['code']) && !$userinfo->is_promoter && $wechatUser['code']){
                //只有扫码才可以获得推广权限
                $data['is_promoter']=isset($wechatUser['isPromoter']) && $wechatUser['isPromoter'] ? 1 : 0;
    ...
    public static function setRoutineUser($routineUser,$spread_uid = 0){
        ...
        $res2 = self::set([
            'account'=>'fy'.$routineUser['uid'].time(),
            'pwd'=>md5(123456),
    ...
    {
        if(isset($orderInfo['combination_id']) && isset($orderInfo['seckill_id']) && isset($orderInfo['bargain_id'])) if(!$orderInfo['combination_id'] || !$orderInfo['seckill_id'] || !$orderInfo['bargain_id']) return true;
