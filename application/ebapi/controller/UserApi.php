...
class UserApi extends AuthController
{
     ...
     * @Modify: Mr. Lin
     * @function: 个人中心
     * @instructions: 只显示type=1的商品，不显示type=2的茶树认购
     * @param {type} 
     * @return: JSON
     */
    public function my(){
        $this->userInfo['collect_product'] = StoreProductRelation::getUserIdCollect($this->userInfo['uid']);;   //获得收藏商品的数量  
        $this->userInfo['collect_merchant'] = StoreMerchantRelation::getUserIdCollect($this->userInfo['uid']);;   //获得关注店铺的数量  
        
        $this->userInfo['orderStatusNum'] = StoreOrder::getOrderStatusNum($this->userInfo['uid']); 
        unset($this->userInfo['pwd']);  //将返回数据中的密码删掉不让密码暴露
        return JsonService::successful($this->userInfo);
    }
    ...
    {
        list($status,$page,$limit,$search)=UtilService::postMore([
            ['status',''],    //发货、未发货、已签收...
            ['page',''],
            ['limit',''],
            ['search',''],
        ],$this->request,true);
        return JsonService::successful(StoreOrder::getUserOrderSearchList($this->uid,$status,$page,$limit,$search));
    }
    ...
    public function get_order($uni = ''){
        ...
        $order['store_name']=Db::table('eb_store_merchant')->where('id','=',$order['mer_id'])->find()['store_name'];
    ...
    public function user_spread_banner_list(){
        ...
        try{
            ...
            foreach ($routineSpreadBanner as $key=>&$item){
                ...
                SystemAttachment::attachmentAdd($posterInfo['name'],$posterInfo['size'],$posterInfo['type'],$posterInfo['dir'],$posterInfo['thumb_path'],1,$posterInfo['image_type'],$posterInfo['time']);
                if($res){
                    if($imageInfo['image_type'] == 1) $item['poster'] = $siteUrl.$posterInfo['dir'];
