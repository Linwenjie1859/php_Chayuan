...
class AuthApi extends AuthController
{
    ...
    {
        return JsonService::successful(StoreCart::getUserProductCartListApi($this->userInfo['uid']));
    }
    ...
    public function confirm_order(Request $request)
    {
        ...
        $priceGroup = StoreOrder::getOrderPriceGroup($cartInfo,$data['type']);
        $other = [
        ...
        if (count($cartIdA) > 1) $seckill_id = 0;
            ...
            if ((int)$seckillinfo['seckill_id'] > 0) $seckill_id = $seckillinfo['seckill_id'];  //秒杀
            else $seckill_id = 0;
        }
    ...
    {
        if (!$productId || !is_numeric($productId)) return JsonService::fail('参数错误');
        $res = StoreCart::setCart($this->userInfo['uid'], $productId, $merId, $cartNum, $uniqueId, 'product');
    ...
    public function now_buy($productId = '', $merId = '', $cartNum = 1, $uniqueId = '', $combinationId = 0, $secKillId = 0, $bargainId = 0)
    {
        ...
        $res = StoreCart::setCart($this->userInfo['uid'], $productId, $merId, $cartNum, $uniqueId, 'product', 1, $combinationId, $secKillId, $bargainId,false);
        if (!$res) return JsonService::fail(StoreCart::getErrorInfo());
        else  return JsonService::successful('ok', ['cartId' => $res->id]);
    }
    /**
    ...
    public function create_order($key = '')
    {
        ...
        list($addressId, $couponId, $payType, $useIntegral, $mark, $combinationId, $pinkId, $seckill_id, $formId, $bargainId,$type,$date,$open_address,$mer_id,$product_id) = UtilService::postMore([
            'addressId', 'couponId', 'payType', 'useIntegral', 'mark',['combinationId', 0], ['pinkId', 0], ['seckill_id', 0], ['formId', ''], ['bargainId', ''],['type',1],'date','open_address','mer_id','product_id'
        ], Request::instance(), true);

        $payType = $payType ? strtolower($payType) : 'weixinapp';
        $production_rate='';        //生产效率

        if($type==2) $production_rate=StoreProduct::where('id',$product_id)->field('production_rate')->find()['production_rate'];
        ...
        $order = StoreOrder::cacheKeyCreateOrder($this->userInfo['uid'], $key, $addressId, $payType, $useIntegral, $couponId, $mark, $combinationId, $pinkId, $seckill_id, $bargainId,$type,$date,$open_address,$production_rate);

        $orderId = $order['order_id'];
        $id= $order['id'];
        $info = compact('orderId', 'key','id');
        if ($orderId) {
    ...
    public function again_order($uni = ''){
        ...
        foreach ($order['cartInfo'] as $v) {
            ...
            else $res[] = StoreCart::setCart($this->userInfo['uid'], $v['product_id'], $v['mer_id'], $v['cart_num'], isset($v['productInfo']['attrInfo']['unique']) ? $v['productInfo']['attrInfo']['unique'] : '', 'product', 0, 0);
        }
        $cateId = [];
        foreach ($res as $v){
